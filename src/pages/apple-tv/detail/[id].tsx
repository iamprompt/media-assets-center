...
import AssetCard from '../../../components/platform/apple-tv/assetcard'
...
export const getServerSideProps: GetServerSideProps = async ({ params }: GetServerSidePropsContext) => {
  ...
  try {
    const {
      data: { payload },
    } = await axios.get<ResponseProps<ProductResultResponse>>('/api/apple-tv/product', {
      params: {
        cId: id,
        country: country,
        locale: locale,
...
const ItemPage: NextPage<{
  ...
  return (
    <SEO
      ...
      image={getImageUrl(result.images.posterArt, { ext: 'jpg', width: 1920 })}
    >
      <Layout>
        <div>
          <div className="aspect-h-9 aspect-w-16 rounded-b-xl">
            <img src={getImageUrl(result.images.posterArt)} alt="" className="rounded-b-xl object-cover" />
          </div>
            ...
            <div className="mt-12 space-y-5">
              ...
              {result.url && (
                <div className="flex justify-center sm:justify-start">
                  <Link
                    href={result.url}
                    className="text-white py-2 px-5 bg-black hover:bg-gray-900 inline-flex items-center space-x-2 rounded-xl border-2 border-gray-300"
                  >
            ...
            {/* <div>
              ...
              <ul className="grid grid-cols-2 md:grid-cols-4 gap-5">
                {Object.entries(result.related || {})
                  ...
                  .map(([id, item]) => (
                    // @ts-expect-error
                    <Card cId={id} d={item} key={id} layout="v" option={{ country, locale }} platform={'apple-tv'} />
                  ))}
              </ul>
            </div> */}
          </div>
